...
import Menu, { CollapsibleMenuItem, MenuItem, MenuLink } from "../components/Menu";
function DefaultLayout() {
    ...
    useEffect(() => {
        async function f() {
            const result = await window.electronAPI.sharedData();
            if (!result.metaData) {
                navigate('/');
            }
    ...
    return (
        <div className="drawer lg:drawer-open">
            ...
            <div className="@container/content relative h-screen overflow-auto drawer-content scrollbar-thin scrollbar-track-base-100 scrollbar-thumb-primary/50" style={{ scrollbarGutter: "stable" }}>
                <nav className="top-0 z-10 sticky bg-gradient-to-b from-base-100/80 to-90% to-base-100/50 shadow-sm backdrop-blur-sm navbar">
                    <div className="flex">
                    ...
                    <Dropdown className="dropdown-end ml-auto">
                        <Dropdown.Button className="btn btn-ghost btn-square m-1">
                        ...
                        <Dropdown.MenuContent className="w-52">
                            ...
                            <CollapsibleMenuItem>
                                <CollapsibleMenuItem.Button>Theme</CollapsibleMenuItem.Button>
                                <CollapsibleMenuItem.Content>
                                    <MenuItem><button onClick={() => setTheme('forest')}>Forest</button></MenuItem>
                                    <MenuItem><button onClick={() => setTheme('dark')}>Dark</button></MenuItem>
                                    <MenuItem><button onClick={() => setTheme('light')}>Light</button></MenuItem>
                                    <MenuItem><button onClick={() => setTheme('bumblebee')}>Bumblebee</button></MenuItem>
                                    <MenuItem><button onClick={() => setTheme('halloween')}>Halloween</button></MenuItem>
                                    <MenuItem><button onClick={() => setTheme('valentine')}>Valentine</button></MenuItem>
                                </CollapsibleMenuItem.Content>
                            </CollapsibleMenuItem>
            ...
            <div className="z-20 shadow backdrop-blur-lg drawer-side">
                ...
                <Menu className="bg-base-200 p-4 w-80 min-h-full [&_li_svg]:size-5 [&_li_svg]:text-primary/60 text-base-content">
                    <div className="bg-stripe-diagonal mb-4 px-3 py-2 border border-base-content/10 rounded-[var(--radius-box)] stripe-color-base-content/5">
                        ...
                        <div className="text-lg">{sharedData.metaData && sharedData.metaData.db_file_name}</div>
                    </div>
                    <MenuLink to="/pages/dashboard" onClick={closeSidebar}>
                    ...
                    <CollapsibleMenuItem open={true}>
                        <CollapsibleMenuItem.Button>
                        ...
                        <CollapsibleMenuItem.Content>
                            <MenuLink to="/pages/accounts" onClick={closeSidebar}>
                            ...
                            <MenuLink to="/pages/transaction" onClick={closeSidebar}>
